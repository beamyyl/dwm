...
static const struct arg args[] = {
	...
	{ run_command,   " | %s",    "echo ''" },
	{ run_command,   "%s",       "p=$(cat /sys/class/power_supply/BAT0/capacity); s=$(cat /sys/class/power_supply/BAT0/status); if [ \"$s\" = \"Charging\" ]; then printf \"󰂈 %d%%\" $p; elif [ \"$s\" = \"Full\" ]; then printf \"󰁹 %d%%\" $p; elif [ $p -le 10 ]; then printf \"󰂎 %d%%\" $p; elif [ $p -le 25 ]; then printf \"󰁺 %d%%\" $p; elif [ $p -le 50 ]; then printf \"󰁼 %d%%\" $p; elif [ $p -le 75 ]; then printf \"󰁾 %d%%\" $p; else printf \"󰂀 %d%%\" $p; fi" },
	{ run_command,   " | %s",    "echo ''" },
